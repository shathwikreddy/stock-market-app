...
interface SplitsTableProps {
    ...
    customSplitIds?: string[];
}

export default function SplitsTable({ events, onDeleteSplit, customSplitIds = [] }: SplitsTableProps) {
    const isCustom = (id: string) => customSplitIds.includes(id);
    const hasCustom = customSplitIds.length > 0;
    ...
    return (
        <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
                ...
                <tbody>
                    {events.length === 0 ? (
                        <tr><td colSpan={hasCustom ? 4 : 3} className="py-4 text-center text-gray-400">No events found.</td></tr>
                    ) : (
                        events.map((item) => (
                            <tr
                                key={item.id}
                            ...
                            >
                                ...
                                <td className={`font-semibold ${isCustom(item.id) ? 'text-blue-600' : 'text-[#1256A0]'}`}>
                                    {item.company}
                                    {isCustom(item.id) && (
                                ...
                                {hasCustom && (
                                    <td className="pr-2">
                                        {isCustom(item.id) && onDeleteSplit && (
                                            <button
                                                onClick={() => onDeleteSplit(item.id)}
