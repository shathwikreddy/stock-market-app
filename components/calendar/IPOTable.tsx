...
interface IPOTableProps {
    events: IPOEvent[];
    onDeleteIPO?: (id: string) => void;
    customIPOIds?: string[];
}

export default function IPOTable({ events, onDeleteIPO, customIPOIds = [] }: IPOTableProps) {
    const isCustom = (id: string) => customIPOIds.includes(id);
    const hasCustom = customIPOIds.length > 0;
                    ...
                    {events.length === 0 ? (
                        ...
                        events.map((item) => (
                            ...
                            >
                                ...
                                {hasCustom && (
                                    <td className="pr-2">
                                        {isCustom(item.id) && onDeleteIPO && (
                                            <button
                                                onClick={() => onDeleteIPO(item.id)}
