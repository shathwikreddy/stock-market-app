...
import {
    ...
    sortOptions,
    BlockDeal,
} from '@/lib/blockDealsMockData';

function formatQuantity(num: number): string {
    return num.toLocaleString('en-IN');
}

interface BlockDealCardProps {
    deal: BlockDeal;
}

function BlockDealCard({ deal }: BlockDealCardProps) {
...
export default function BlockDealsPage() {
    ...
    const filteredData = useMemo(() => {
        let result = blockDealsData.filter((deal) => {
            if (searchQuery) {
            ...
            <div className="max-w-7xl mx-auto px-4 py-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filteredData.map((deal) => (
                        <BlockDealCard key={deal.id} deal={deal} />
                    ))}
                </div>

                {filteredData.length === 0 && (
                    <div className="bg-white border border-gray-200 rounded-lg p-8 text-center">
                        <p className="text-gray-500">No block deals found matching your criteria.</p>
                    </div>
                )}
            </div>
        </div>
