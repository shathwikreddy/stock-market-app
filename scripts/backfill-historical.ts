...
// ── Config ──
const CONCURRENT = parseInt(process.env.BACKFILL_CONCURRENT || '1'); // Historical API is rate-limited per Dhan user
const DELAY_MS = parseInt(process.env.BACKFILL_DELAY_MS || '5000');
const MIN_DATA_POINTS = 10;   // Stocks with fewer points need backfill
const API_TIMEOUT = parseInt(process.env.BACKFILL_API_TIMEOUT_MS || '20000');
const MAX_ATTEMPTS = parseInt(process.env.BACKFILL_MAX_ATTEMPTS || '5');
const RATE_LIMIT_COOLDOWN_MS = parseInt(process.env.BACKFILL_RATE_LIMIT_COOLDOWN_MS || '180000');
...
async function fetchHistoricalWithRetry(
  ...
  fromDate: string,
  toDate: string
): Promise<{ securityId: number; exchangeSegment: string; closes: number[] | null; timestamps: number[] | null } | null> {
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      const err = e as { response?: { status?: number; data?: { errorCode?: string; errorMessage?: string } }; message?: string };
      if (err.response?.status === 429) {
        const message = err.response.data?.errorMessage || 'rate limited';
        console.error(`  Rate limited on ${securityId}/${exchangeSegment}: ${message}. Cooling down ${Math.round(RATE_LIMIT_COOLDOWN_MS / 1000)}s.`);
...
async function main() {
  ...
  const needBackfill = await prisma.$queryRawUnsafe<Array<{
    ...
    ORDER BY sm.exchange DESC, sm."securityId" ASC
  `);
  ...
  for (let i = 0; i < needBackfill.length && !stopped; i += CONCURRENT) {
    ...
    const results = await Promise.allSettled(
      batch.map(async ({ securityId, exchangeSegment }) => {
        return fetchHistoricalWithRetry(securityId, exchangeSegment, fromDate, toDate);
      })
    );
    ...
    for (const r of results) {
      ...
      if (!closes || closes.length === 0 || !timestamps || timestamps.length === 0) {
