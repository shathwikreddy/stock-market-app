...
function allowRequestTriggeredSync(): boolean {
  return process.env.MARKET_REQUEST_SYNC === 'true';
}
 ...
 * If request-triggered sync is enabled, this can bootstrap/refresh data.
 * In production, scheduled/authenticated jobs should populate data instead.
 */
export async function ensureDataReady(exchange: Exchange): Promise<void> {
  ...
  if (count === 0) {
    if (!allowRequestTriggeredSync()) {
      throw new Error(
        `Market data is not initialized for ${exchange}. Run the authenticated sync/bootstrap job first.`
      );
    }

    // Development escape hatch: blocking sync if explicitly enabled.
  ...
  if (latest && Date.now() - new Date(latest.syncedAt).getTime() > 120_000 && allowRequestTriggeredSync()) {
